...
GAME_IDS = [
    ...
    "ODD_ONE_OUT","TITLE_FIGHT","ALIBI_DROP","HYPE_OR_YIKE","SCATTERBLAST","MAJORITY_REPORT"
]
...
def update_min_humor(target: float, step: float) -> bool:
    ...
    for gid, pr in pass_rates.items():
        ...
        block_re = re.compile(rf"(GameIds\.{re.escape(const)}\s*to\s*QualityProfile\([^\)]*?minHumor\s*=\s*)([0-9.]+)")
