...
class SystemSanityCheck:
    ...
    def check_1_game_metadata(self):
        ...
        for legacy in legacy_games:
            if legacy in content and '// REMOVED' not in content[content.find(legacy):content.find(legacy)+100]:
    ...
    def check_3_template_files(self):
        ...
        required_templates = [
            'roast_consensus.json', 'confession_or_cap.json', 'poison_pitch.json',
            'fill_in_finisher.json', 'red_flag.json', 'hot_seat_imposter.json',
            'text_trap.json', 'taboo_timer.json', 'the_unifying_theory.json',
            'title_fight.json', 'alibi_drop.json', 'reality_check.json',
            'scatterblast.json', 'over_under.json'
        ]
    ...
    def check_4_no_legacy_references(self):
        ...
        legacy_patterns = ['MAJORITY_REPORT', 'ODD_ONE_OUT', 'HYPE_OR_YIKE', 
                          'GameIds.MAJORITY', 'GameIds.ODD_ONE', 'GameIds.HYPE_YIKE']
                ...
                for pattern in legacy_patterns:
                    if pattern in line:
                        ...
                        if stripped.startswith('//') or '/*' in line or '*/' in line:
                            ...
                            active_references += 1
                            self.warnings.append(f"{kt_file.name}:{i+1} - Active reference: {pattern}")
